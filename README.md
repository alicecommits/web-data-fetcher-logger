...
- *Other note: the loop keywords `innerloop` and `outerloop`, describing the `try` outer block and `while` inner block respectively, were used in a preliminary version of the code alongwith `break innerloop;` and `break outerloop;`. They were kept for readability only.*
